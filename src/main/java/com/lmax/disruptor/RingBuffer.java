...
abstract class RingBufferFields<E> extends RingBufferPad {
    ...
    /**
     * RingBuffer中真正装数据的数组，和ArrayBlockingQueue循环阻塞队列做个对比
     */
    private final Object[] entries;

    protected final int bufferSize;
    protected final Sequencer sequencer;
    private static final Logger logger = LoggerFactory.getLogger(RingBufferFields.class);
    ...
    RingBufferFields(EventFactory<E> eventFactory, Sequencer sequencer) {
        ...
        //2的幂－1的目的为方便使用&为操作定位槽位下标
        this.indexMask = bufferSize - 1;
        this.entries = new Object[sequencer.getBufferSize() + 2 * BUFFER_PAD];
        logger.error("RingBuffer开始初始化所有槽位");
...
public final class RingBuffer<E> extends RingBufferFields<E> implements Cursored, EventSequencer<E>, EventSink<E> {
    ...
    public SequenceBarrier newBarrier(Sequence... sequencesToTrack) {
        //此处的sequencer可能是SingleProducerSequencer，也可能是多生产者的MultiProducerSequencer
        //SequenceBarrier是一个接口，最终new的是 ProcessingSequenceBarrier
        SequenceBarrier sequenceBarrier = sequencer.newBarrier(sequencesToTrack);
    ...
    public <A> void publishEvent(EventTranslatorOneArg<E, A> translator, A arg0) {
        final long sequence = sequencer.next();
        try {
            //通过UnSafe操作数组获取sequence对应的槽位对象
            E e = get(sequence);
            translator.translateTo(e, sequence, arg0);
        } finally {
            sequencer.publish(sequence);
        }
    }
