...
public class Disruptor<T> {
    ...
    public Disruptor(final EventFactory<T> eventFactory,
        ...
        logger.error("Disruptor构造函数开始，先创建RingBuffer，再创建BasicExecutor");
        this.ringBuffer = RingBuffer.create(producerType, eventFactory, ringBufferSize, waitStrategy);
        this.executor = new BasicExecutor(threadFactory);
    }
    ...
    //使用BatchEventProcessor构建消费者链时的逻辑都在createEventProcessors这个方法中。
    ...
    EventHandlerGroup<T> createEventProcessors(final Sequence[] barrierSequences, final EventHandler<? super T>[] eventHandlers) {
        ...
        final Sequence[] processorSequences = new Sequence[length];

        //同一级别的消费者当然要共用一个SuquenceBarrier，BatchEventProceesor可以执行批量处理
        final SequenceBarrier barrier = ringBuffer.newBarrier(barrierSequences);

        for (int i = 0; i < length; i++) {
            final EventHandler<? super T> eventHandler = eventHandlers[i];
            //从这里可以看出，就算是同一组内的EventHandler，也是单独将其包装为一个BatchEventHandler，BatchEventHandler本身就是最小单位了。
            // EventProcessor的本质是一个Runable线程可执行体，为什么叫Batchxxx？因为run方法内部在proccessEvents()可以批量地取出事件执行。
            // 具体在执行时同一组内的每个Handler都会对应仅一条的线程进行执行。（当然也可以配置成多线程）
            //Disruptor推荐是生产者使用单线程，每个消费者EventHandler也使用单线程。
            ...
            //每一个Handler对应的BatchEventProcessor内部都有一个自己的sequence，互不干扰。
            processorSequences[i] = batchEventProcessor.getSequence();
        }
        // 每次添加完事件处理器后，更新门控序列，以便后续调用链的添加。（所谓门控，是指后续消费链的消费，不能超过前边。）
        updateGatingSequencesForNextInChain(barrierSequences, processorSequences);
        logger.error("调用handleEventsWith()创建完processorSequences的内容是:{}", processorSequences);
        //todo:注意看这时的processorSequences数组是什么样的，EventHandlerGroup会持有它。
